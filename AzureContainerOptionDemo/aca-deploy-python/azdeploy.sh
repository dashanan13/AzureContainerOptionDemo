...
rg="mohit-ContainerOptions"  # Resource Group name
location="swedencentral"   # Azure region for the resources
...
create_acr_and_build_image() {
    ...
    #
    az acr build --resource-group $rg --registry $acr_name --image $container_image --file api/Dockerfile api/ > /dev/null 2>&1
    #az acr build --resource-group mohit-ContainerOptions --registry acrab95ccdb --image "ai-api:v1" --file api/Dockerfile api/
